...
# Use numpy indexing to place the resized piano image at the specified position
		...
		if results.left_hand_landmarks: #if image contains landmarks
			landmarks = results.left_hand_landmarks.landmark
			if int(landmarks[8].x * image.shape[1]) < 160 and int(landmarks[8].y * image.shape[0]) < 90:
				playc("C")
			elif int(landmarks[8].x * image.shape[1]) > 1120 and int(landmarks[8].y * image.shape[0]) < 90:
				playc("F")
			elif int(landmarks[8].x * image.shape[1]) < 160 and int(landmarks[8].y * image.shape[0]) > 630:
				playc("Am")
			elif int(landmarks[8].x * image.shape[1]) > 1120 and int(landmarks[8].y * image.shape[0]) > 630:
				playc("As")
